...
class KronchESProvider: MainAPI() {
   ...
   /* private suspend fun proxyToken(): Map<String, String> {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val ssss =
            aaaa.items
                .filter { it.episodeMetadata?.audioLocale == "pt-BR" || it.episodeMetadata?.audioLocale == "pt-BR" }
                .map {
        ...
        val res =
            app.get(
                "$krunchyapi/content/v1/browse?locale=pt-BR&n=30&sort_by=popularity",
                headers = latestKrunchyHeader
            ).parsed<KrunchyHome>()
        ...
        if (epss.isNotEmpty()) items.add(HomePageList("Novos episodios (LEG)", epss, true))
        if (ssss.isNotEmpty()) items.add(HomePageList("Novos episodios (DUB)", ssss, true))
    ...
    private suspend fun getMovie(id: String?):ArrayList<Episode> {
        ...
        val metadainfo = app.get("$krunchyapi/content/v2/cms/movie_listings/$id/movies?locale=pt-BR", headers = latestKrunchyHeader).parsed<KrunchyLoadMain>()
        metadainfo.data.map {
            val title = it.title
            val epID = it.id
    ...
    private suspend fun getRecommendations(seriesId: String?): List<SearchResponse>?{
        proxyToken()
        val recsurl = "$krunchyapi/content/v2/discover/similar_to/$seriesId?locale=pt-BR&n=30"
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tttt = if (isMovie) "$krunchyapi/content/v2/cms/movie_listings/$seriesIDSuper?locale=pt-BR" else "$krunchyapi/content/v2/cms/series/$seriesIDSuper?locale=pt-BR"
        ...
        if (tvType == TvType.Anime) {
            ...
            val nn =
                app.get(
                    "${krunchyapi}/content/v2/cms/series/$seriesIDSuper/seasons?locale=pt-BR",
                    headers = latestKrunchyHeader
                )
                    .parsedSafe<BetaKronch>()
                    ?: throw ErrorLoadingException("Couldn't get episodes, try again")
            if (nn.data.isEmpty())
                throw ErrorLoadingException("Falha ao obter os episodios, tente outra vez")
            val inn =
                nn.data.filter {
                    !it.title!!.contains(
            ...
            inn.apmap { main ->
                ...
                val res =
                    app.get(
                        "${krunchyapi}/content/v2/cms/seasons/$mainID/episodes?&locale=pt-BR",
                        headers = latestKrunchyHeader
                    )
                        .parsedSafe<BetaKronch>()
                        ?: throw ErrorLoadingException("Couldn't get episodes, try again")
                if (res.data.isEmpty())
                    throw ErrorLoadingException(
                        "Falha ao obter a temporada ${main.title} corretamente, tente outra vez"
                    )
                val restwo =
                    res.data.filter {
                        it.audioLocale == "ja-JP" ||
            ...
            innversions.map { ve ->
                val versionsfiltered =
                    ve.versions?.filter {
                        (it.audioLocale?.contains(Regex("ja-JP|zh-CN|pt-BR")) == true ||
                ...
                versionsfiltered?.apmap { vers ->
                    ...
                    val resv =
                        app.get(
                            "${krunchyapi}/content/v2/cms/seasons/$guid/episodes?&locale=pt-BR",
                            headers = latestKrunchyHeader
                        )
                            .parsedSafe<BetaKronch>()
                            ?: throw ErrorLoadingException("Couldn't get episodes, try again")
                    if (resv.data.isEmpty())
                        throw ErrorLoadingException(
                            "Falha ao obter a temporada ${ve.title} corretamente, tente outra vez"
                        )
                    resv.data.map { pss ->
            ...
            bbb.apmap { aa ->
                aa.entries.filter {
                    ...
                    if (testtting.contains(Regex("(?i)accessdenied"))) handler.postFunction {
                        context.let { tt -> Toast.makeText(tt, "Recarregar links.", Toast.LENGTH_LONG).show() }
                    } else {
                        val raw = str.hardsubLocale?.isEmpty()
                        val hardsubinfo = str.hardsubLocale?.contains("pt-BR")
                        val vvv = if (str.url!!.contains("vrv.co")) "_VRV" else ""
                        val name = if (raw == false && issub) "Kronch$vvv Hardsub Português (BR)" else if (raw == true && issub) "Kronch$vvv RAW" else "Kronch$vvv Português"
